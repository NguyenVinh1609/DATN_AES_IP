...
void AES128_ECB_decrypt_dpi(svBitVec32 *input, const svBitVec32 *key, svBitVec32 *output)
{
  uint8_t *decryptInput;
  uint8_t *decryptKey;
  uint8_t *decryptOutput;
  uint8_t i;

  decryptInput = (uint8_t *)malloc(sizeof(uint8_t) * KEYLEN);
  decryptKey = (uint8_t *)malloc(sizeof(uint8_t) * KEYLEN);
  decryptOutput = (uint8_t *)malloc(sizeof(uint8_t) * KEYLEN);

  for (i = 0; i < KEYLEN; ++i)
  {
    decryptInput[i] = (uint8_t)input[i];
    decryptKey[i] = (uint8_t)key[i];
  }

  AES128_ECB_decrypt(decryptInput, decryptKey, decryptOutput);

  for (i = 0; i < KEYLEN; ++i)
  {
    output[i] = decryptOutput[i];
  }
}
